    ...
    React.createElement('div', { className: "min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-3 sm:p-6" },
      React.createElement('div', { className: "max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-4 sm:p-8" },
        React.createElement('div', { className: "text-center mb-6 sm:mb-8" },
          ...
          React.createElement('p', { className: "text-sm sm:text-base text-gray-600" }, '专业心理健康评估工具')
        ),
        ...
        React.createElement('div', { className: "space-y-4 sm:space-y-6 text-gray-700 leading-relaxed text-sm sm:text-base" },
          React.createElement('p', { className: "text-base sm:text-lg" },
            'SCL-90（Symptom Checklist 90）是一种常用的心理健康自评量表，包含90个项目，涉及感觉、情感、思维、意识、行为、生活习惯、人际关系、饮食睡眠等多个方面。'
          ),

          React.createElement('div', { className: "bg-blue-50 border border-blue-200 rounded-lg p-4 sm:p-6" },
            React.createElement('h3', { className: "text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-blue-800" }, '⚠️ 重要提醒'),
            React.createElement('ul', { className: "space-y-2 text-blue-700" },
              React.createElement('li', null, '• 量表只是辅助评估工具，可以帮助了解自己的情绪状况'),
              React.createElement('li', null, '• 测试结果仅供参考，不代表实际诊断'),
              React.createElement('li', null, '• 如有严重心理困扰，请及时寻求专业医疗帮助')
            )
          ),

          React.createElement('p', null,
            '请仔细阅读以下每一条描述，并根据您在', React.createElement('strong', null, '最近一周内'), '的实际感受，选择最适合的答案。'
          ),

          React.createElement('div', { className: "bg-gray-50 p-4 sm:p-6 rounded-lg" },
            React.createElement('h3', { className: "text-lg sm:text-xl font-semibold mb-3 sm:mb-4" }, '评分标准：'),
            React.createElement('div', { className: "grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3" },
              React.createElement('div', { className: "flex items-center space-x-2" },
                ...
                React.createElement('span', { className: "font-medium" }, '从无：'),
                React.createElement('span', { className: "text-sm" }, '没有或很少时间有')
              ),
              React.createElement('div', { className: "flex items-center space-x-2" },
                ...
                React.createElement('span', { className: "font-medium" }, '很轻：'),
                React.createElement('span', { className: "text-sm" }, '少部分时间有')
              ),
              React.createElement('div', { className: "flex items-center space-x-2" },
                ...
                React.createElement('span', { className: "font-medium" }, '中等：'),
                React.createElement('span', { className: "text-sm" }, '相当多时间有')
              ),
              React.createElement('div', { className: "flex items-center space-x-2" },
                ...
                React.createElement('span', { className: "font-medium" }, '偏重：'),
                React.createElement('span', { className: "text-sm" }, '绝大部分时间有')
              ),
              React.createElement('div', { className: "flex items-center space-x-2 sm:col-span-2" },
                ...
                React.createElement('span', { className: "font-medium" }, '严重：'),
                React.createElement('span', { className: "text-sm" }, '全部时间都有')
              )
            )
          )
        ),
        ...
        React.createElement('div', { className: "mt-6 sm:mt-8 text-center" },
          ...
          showQuickTestBtn && React.createElement('div', { className: "mt-4 p-4 bg-purple-100 border-2 border-purple-300 rounded-lg" },
            React.createElement('p', { className: "text-sm text-purple-700 mb-3 font-semibold" }, '🎯 快速测试模式已解锁！'),
            React.createElement('button', {
      ...
      React.createElement('div', { className: "max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-4 sm:p-8" },
        React.createElement('div', { className: "text-center mb-6 sm:mb-8" },
          ...
          React.createElement('div', { className: "text-gray-600" },
            React.createElement('span', { className: "text-lg sm:text-xl font-semibold" }, `第 ${currentQuestion + 1} 题`),
            React.createElement('span', { className: "ml-2 sm:ml-4 text-sm sm:text-base" }, '共 90 题')
          ),
      ...
      React.createElement('div', { className: "max-w-6xl mx-auto bg-white rounded-2xl shadow-xl p-4 sm:p-8" },
          ...
          React.createElement('div', { className: "text-center" },
            ...
            React.createElement('div', { className: "flex flex-col sm:flex-row justify-center items-center space-y-2 sm:space-y-0 sm:space-x-6 mb-3 sm:mb-4" },
              React.createElement('div', { className: "text-center" },
                ...
                React.createElement('div', { className: "text-xs sm:text-sm text-gray-600" }, '总分')
              ),
              React.createElement('div', { className: "text-center" },
                React.createElement('div', { className: "text-2xl sm:text-4xl font-bold text-gray-800" }, results.avgScore),
                React.createElement('div', { className: "text-xs sm:text-sm text-gray-600" }, '平均分')
              )
            ),
        ...
        React.createElement('div', { className: "grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-6 sm:mb-8" },
          React.createElement('div', { className: "bg-gray-50 rounded-xl p-4 sm:p-6" },
            React.createElement('h3', { className: "text-lg sm:text-xl font-bold text-gray-800 mb-4 text-center" }, '各维度评估图'),
          ...
          React.createElement('div', { className: "space-y-3 sm:space-y-4" },
            React.createElement('h3', { className: "text-lg sm:text-xl font-bold text-gray-800 mb-4" }, '详细因子分析'),
        ...
        React.createElement('div', { className: "space-y-4 sm:space-y-6" },
          React.createElement('h3', { className: "text-xl sm:text-2xl font-bold text-gray-800" }, '详细分析与建议'),
            ...
            Object.keys(factors).map(factor => {
              ...
              if (parseFloat(avgScore) >= 2.0) {
                return React.createElement('div', {
                  ...
                  React.createElement('div', null,
                    React.createElement('h5', { className: "font-semibold text-gray-800 mb-2 text-sm" }, '💡 改善建议：'),
        ...
        React.createElement('div', { className: "mt-6 sm:mt-8 bg-gray-50 rounded-xl p-4 sm:p-6" },
          React.createElement('h4', { className: "font-bold text-gray-800 mb-3 text-sm sm:text-base" }, '📊 评分标准说明：'),
          React.createElement('div', { className: "grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm" },
            React.createElement('div', { className: "flex items-center space-x-2" },
              ...
              React.createElement('span', null, React.createElement('strong', null, '良好：'), ' 1.0-2.0分')
            ),
            React.createElement('div', { className: "flex items-center space-x-2" },
              React.createElement('span', { className: "w-3 h-3 bg-yellow-500 rounded" }),
              React.createElement('span', null, React.createElement('strong', null, '轻微：'), ' 2.0-3.0分')
            ),
            React.createElement('div', { className: "flex items-center space-x-2" },
              React.createElement('span', { className: "w-3 h-3 bg-orange-500 rounded" }),
              React.createElement('span', null, React.createElement('strong', null, '中度：'), ' 3.0-4.0分')
            ),
            React.createElement('div', { className: "flex items-center space-x-2" },
              React.createElement('span', { className: "w-3 h-3 bg-red-500 rounded" }),
              React.createElement('span', null, React.createElement('strong', null, '重度：'), ' 4.0-5.0分')
            )
          ),
          ...
          React.createElement('div', { className: "mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg" },
            React.createElement('p', { className: "text-xs sm:text-sm text-gray-700 leading-relaxed" },
              React.createElement('strong', null, '参考标准：'),
              '按中国常模结果，SCL-90总分超过160分或单项均分超过2分建议进一步检查；总分超过200分说明有明显心理问题，可寻求心理咨询；超过250分则较为严重，需要详细医学检查和专业治疗。'
            )
          )
        ),

        React.createElement('div', { className: "mt-6 sm:mt-8 bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl p-4 sm:p-6" },
